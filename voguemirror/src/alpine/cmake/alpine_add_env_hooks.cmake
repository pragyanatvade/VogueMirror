...
# ``etc/alpine/profile.d/``, where it will be read by global generated
# ``setup.<shell>``.
#
...
# Example::
#
#   alpine_add_env_hooks(my_prefix SHELLS bash tcsh zsh DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}/env-hooks)
#
...
# @public
#
function(alpine_add_env_hooks file_prefix)
  cmake_parse_arguments(ARG "SKIP_INSTALL" "DIRECTORY" "SHELLS" ${ARGN})

  # create directory if necessary
  if(NOT IS_DIRECTORY ${ALPINE_DEVEL_PREFIX}/etc/alpine/profile.d)
    file(MAKE_DIRECTORY ${ALPINE_DEVEL_PREFIX}/etc/alpine/profile.d)
  endif()
  ...
  foreach(shell ${ARG_SHELLS})
    ...
    if(EXISTS ${base}.em OR EXISTS ${base}.develspace.em)
      ...
      em_expand(${alpine_EXTRAS_DIR}/templates/env-hook.context.py.in
        ${CMAKE_CURRENT_BINARY_DIR}/alpine_generated/${ENV_HOOK}.develspace.context.py
        ${em_template}
        ${ALPINE_DEVEL_PREFIX}/etc/alpine/profile.d/${ENV_HOOK})
    ...
    elseif (EXISTS ${base})
      # copy plain file
      file(COPY ${base} DESTINATION ${ALPINE_DEVEL_PREFIX}/etc/alpine/profile.d)
    else()
      message(FATAL_ERROR "alpine_add_env_hooks() could not find environment hook.  Either '${base}', '${base}.em', '${base}.develspace.em' or '${base}.in' must exist.")
    endif()
    ...
    if(EXISTS ${base}.em OR EXISTS ${base}.installspace.em)
      ...
      em_expand(${alpine_EXTRAS_DIR}/templates/env-hook.context.py.in
        ${CMAKE_CURRENT_BINARY_DIR}/alpine_generated/${ENV_HOOK}.installspace.context.py
        ${em_template}
        ${CMAKE_CURRENT_BINARY_DIR}/alpine_generated/installspace/${ENV_HOOK})
      if(NOT ${ARG_SKIP_INSTALL})
        install(FILES ${CMAKE_CURRENT_BINARY_DIR}/alpine_generated/installspace/${ENV_HOOK}
          DESTINATION etc/alpine/profile.d)
      endif()
