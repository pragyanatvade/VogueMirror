...
# The signature:
#
#   alpine_install_python(PROGRAMS files... DESTINATION <dir>
#     [OPTIONAL]
#   )
#
...
#
#
function(alpine_install_python signature)
  string(TOUPPER "${signature}" signature)
  if(NOT "${signature}" STREQUAL "PROGRAMS")
    message(FATAL_ERROR "alpine_install_python() only supports the PROGRAMS signature (not '${signature}').")
  endif()
  cmake_parse_arguments(ARG "OPTIONAL" "DESTINATION" "" ${ARGN})
  if(NOT ARG_DESTINATION)
    message(FATAL_ERROR "alpine_install_python() called without required DESTINATION argument.")
  endif()
  foreach(file ${ARG_UNPARSED_ARGUMENTS})
    ...
    if(EXISTS ${file})
      ...
      if("${shebang_line}" STREQUAL "${prefix}")
        ...
        set(file "${CMAKE_CURRENT_BINARY_DIR}/alpine_generated/installspace")
    ...
    elseif(NOT ARG_OPTIONAL)
      message(FATAL_ERROR "alpine_install_python() called with non-existing file '${file}'.")
    endif()
  endforeach()
endfunction()
