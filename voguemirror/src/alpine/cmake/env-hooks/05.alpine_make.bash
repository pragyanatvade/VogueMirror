function _alpine_make()
{
    ...
    if [[ "$cur" == -DCMAKE_BUILD_TYPE=* ]]; then
    ...
    elif [[ "$cur" == -DALPINE_ENABLE_TESTING=* ]]; then
        # autocomplete alpine argument ALPINE_ENABLE_TESTING with its options
        COMPREPLY=( $( compgen -P "-DALPINE_ENABLE_TESTING=" -W "0 1" -- "${cur:24}" ) )
    elif [[ "$cur" == -DALPINE_DEVEL_PREFIX=* || "$cur" == -DCMAKE_INSTALL_PREFIX=* ]]; then
        COMPREPLY=()
    elif [[ "$cur" == -* ]]; then
        ...
        if [[ "$cur" == -* ]]; then
            ...
            opts="$opts -DALPINE_DEVEL_PREFIX= -DALPINE_ENABLE_TESTING= -DCMAKE_INSTALL_PREFIX= -DCMAKE_BUILD_TYPE="
        fi
...
complete -F _alpine_make alpine_make
