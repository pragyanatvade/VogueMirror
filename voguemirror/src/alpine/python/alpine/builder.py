...
try:
    from alpine_pkg.cmake import configure_file, get_metaproject_cmake_template_path
    from alpine_pkg.projects import find_projects
    from alpine_pkg.topological_order import topological_order_projects
except ImportError as e:
    sys.exit(
        'ImportError: "from alpine_pkg.topological_order import '
        'topological_order" failed: %s\nMake sure that you have installed '
        '"alpine_pkg", it is up to date and on the PYTHONPATH.' % e
    )

from alpine.cmake import get_cmake_path
from alpine.terminal_color import ansi, disable_ANSI_colors, fmt, sanitize
...
def extract_cmake_and_make_arguments(args):
    args, cmake_args, make_args, _ = _extract_cmake_and_make_arguments(args, extract_alpine_make=False)
    return args, cmake_args, make_args


def extract_cmake_and_make_and_alpine_make_arguments(args):
    return _extract_cmake_and_make_arguments(args, extract_alpine_make=True)


def _extract_cmake_and_make_arguments(args, extract_alpine_make):
    cmake_args = []
    make_args = []
    alpine_make_args = []
    ...
    if extract_alpine_make:
        arg_types['--alpine-make-args'] = alpine_make_args
    ...
    return args, implicit_cmake_args + cmake_args, make_args, alpine_make_args
...
def get_python_install_dir():
    # this function returns the same value as the CMake variable PYTHON_INSTALL_DIR from alpine/cmake/python.cmake
...
def build_alpine_project(
    ...
    cprint(
        "Processing @{cf}alpine@| project: '@!@{bf}" +
        package.name + "@|'"
    )
    ...
    if not os.path.exists(makefile) or force_cmake:
        ...
        if not os.path.exists(os.path.join(package_dir, 'CMakeLists.txt')):
            ...
            if 'metaproject' not in export_tags:
                print(colorize_line('Error: Project "%s" does not have a CMakeLists.txt file' % package.name))
                sys.exit('Can not build alpine project without CMakeLists.txt file')
            # generate CMakeLists.txt for metpackages without one
            print(colorize_line('Warning: metaproject "%s" should have a CMakeLists.txt file' % package.name))
            cmake_code = configure_file(
                get_metaproject_cmake_template_path(),
                {'name': package.name, 'metaproject_arguments': 'DIRECTORY "%s"' % package_dir})
        ...
        cmake_cmd = [
            'cmake',
            package_dir,
            '-DALPINE_DEVEL_PREFIX=' + develspace,
    ...
    if install:
        if has_make_target(build_dir, 'install'):
            ...
            print(fmt('@{yf}Project has no "@{boldon}install@{boldoff}" target, skipping "make install" invocation...'))
...
def get_additional_environment(install, destdir, installspace):
    ...
    if install and destdir:
        add_env['_ALPINE_SETUP_DIR'] = os.path.join(destdir, installspace[1:])
    return add_env
...
def build_cmake_package(
    ...
    cprint(
        "Processing @{cf}plain cmake@| project: '@!@{bf}" + package.name +
        "@|'"
    )
    ...
    # Generate env.sh for chaining to alpine projects
...
""".format(**variables))
    ...
    # Generate setup.sh for chaining to alpine projects
...
if [ -z "$ALPINE_SHELL" ]; then
  ALPINE_SHELL=sh
fi
""")
            if last_env is not None:
                file_handle.write('. "%s.$ALPINE_SHELL"\n\n' % last_setup_env[:-3])
            file_handle.write("""\
...
def build_package(
    path, package,
    workspace, buildspace, develspace, installspace,
    install, force_cmake, quiet, last_env, cmake_args, make_args, alpine_make_args,
    destdir=None,
    number=None, of=None
):
    if platform.system() in ['Linux', 'Darwin']:
        status_msg = '{project_name} [{number} of {total}]'.format(project_name=package.name, number=number, total=of)
    ...
    if build_type == 'alpine':
        build_alpine_project(
            path, package,
            workspace, buildspace, develspace, installspace,
            install, force_cmake, quiet, last_env, cmake_args, make_args + alpine_make_args,
            destdir=destdir
        ...
        if not os.path.exists(new_last_env):
            raise RuntimeError(
                ...
                "'\n  This sometimes occurs when a non-alpine project is "
                "interpreted as a alpine project.\n  This can also occur "
    ...
    if number is not None and of is not None:
        ...
        msg = ''
    cprint('@{gf}<==@| Finished processing project' + msg + ': \'@{bf}@!' +
           package.name + '@|\'')
    return new_last_env
...
def _print_build_error(package, e):
    ...
    cprint('@{rf}@!<==@| Failed to process project \'@!@{bf}' + package.name + '@|\': \n  ' + e_msg)
...
def cmake_input_changed(packages, build_path, cmake_args=None, filename='alpine_make'):
    # get current input
...
def get_project_names_with_recursive_dependencies(packages, pkg_names):
    dependencies = set([])
    ...
    while check_pkg_names:
        ...
        if pkg_name in packages_by_name:
            ...
            for dep in [dep.name for dep in (pkg.builddeps + pkg.buildtooldeps + pkg.rundeps)]:
