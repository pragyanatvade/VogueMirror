...
try:
    from alpine.init_workspace import init_workspace, _symlink_or_copy
...
class InitWorkspaceTest(unittest.TestCase):
    ...
    def test_init_workspace(self):
        try:
            ...
            os.makedirs(join(root_dir, 'ws1', 'alpine'))
            os.makedirs(join(root_dir, 'ws1', 'alpine', 'cmake'))
            with open(join(root_dir, 'ws1', 'alpine', 'cmake', 'toplevel.cmake'),
                      'ab') as fhand:
                fhand.write(''.encode('UTF-8'))
            with open(join(root_dir, 'ws1', '.alpine'), 'ab') as fhand:
                fhand.write(''.encode('UTF-8'))
            os.makedirs(join(root_dir, 'ws2'))
            with open(join(root_dir, 'ws2', '.alpine'), 'ab') as fhand:
                fhand.write(''.encode('UTF-8'))
            ...
            self.assertEqual(
                join('alpine', 'cmake', 'toplevel.cmake'),
