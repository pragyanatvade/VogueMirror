...
try:
    from alpine.workspace import get_workspaces, get_source_paths

    from alpine.workspace import ALPINE_MARKER_FILE
    import alpine.workspace
...
class WorkspaceTest(unittest.TestCase):
    ...
    def test_get_workspaces(self):
        try:
            ...
            alpine.workspace.os.environ = {}
            self.assertEqual([], get_workspaces())
            alpine.workspace.os.environ = {'CMAKE_PREFIX_PATH': ''}
            self.assertEqual([], get_workspaces())
            alpine.workspace.os.environ = {'CMAKE_PREFIX_PATH': ws1}
            self.assertEqual([ws1], get_workspaces())
            alpine.workspace.os.environ = {'CMAKE_PREFIX_PATH': 'nowhere'}
            self.assertEqual([], get_workspaces())
            alpine.workspace.os.environ = {'CMAKE_PREFIX_PATH': ws2 + os.pathsep + ws1}
            self.assertEqual([ws2, ws1], get_workspaces())
        finally:
            shutil.rmtree(root_dir)
            alpine.workspace.os.environ = os.environ
