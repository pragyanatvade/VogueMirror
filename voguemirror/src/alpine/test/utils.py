...
ALPINE_DIR = os.path.dirname(TESTS_DIR)
...
def vminstall(pth, specfile):
    '''
    calls vminstall in pth with given specfile,
    then replaces CMakelists with alpine's toplevel.cmake'
    '''
    assert os.path.exists(specfile), specfile
    # to save testing time, we do not invoke vminstall when we
    # already have a .vminstall file
    if not os.path.exists(os.path.join(pth, '.vminstall')):
        succeed(["vminstall", "-j8", "--alpine", "-n",
...
def create_alpine_workspace(pth):
    """
    prepares path to be a alpine workspace, by copying alpine and
    creating a CMakeLists.txt
    """
    if not os.path.isdir(pth):
        os.makedirs(pth)

    alpine_dir = os.path.join(pth, 'alpine')
    if os.path.isdir(alpine_dir):
        shutil.rmtree(alpine_dir)
    # copy current alpine sources into workspace
    ...
    def notest(folder, contents):
        if folder.endswith('test'):
            return ['tmp']
        return []
    shutil.copytree(ALPINE_DIR, alpine_dir, symlinks=True, ignore=notest)
    assert (os.path.exists(pth + "/alpine/cmake/toplevel.cmake")), \
        pth + "/alpine/cmake/toplevel.cmake"
    # workaround for current vminstall creating flawed CMakelists
    workspace_cmake = os.path.join(pth, "CMakeLists.txt")
    if os.path.isfile(workspace_cmake):
        os.remove(workspace_cmake)
    succeed(["/bin/ln", "-s", "alpine/cmake/toplevel.cmake",
             "CMakeLists.txt"],
            cwd=pth)
...
class AbstractAlpineWorkspaceTest(unittest.TestCase):
    ...
    build folder, and a src folder with latest alpine from source.
    """

    def __init__(self, testCaseName, rootdir=None):
        super(AbstractAlpineWorkspaceTest, self).__init__(testCaseName)
    ...
    def setupWorkspaceContents(self):
        create_alpine_workspace(self.workspacedir)
    ...
    def cmake(self,
        ...
        if 'ALPINE_DPKG_BUILDPROJECT_FLAGS' not in kwargs:
            kwargs['ALPINE_DPKG_BUILDPROJECT_FLAGS'] = '-d;-S;-us;-uc'
