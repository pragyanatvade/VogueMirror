#ifndef VM_SCANNER_CUDA_DEVICE_MEMORY_HPP
#define VM_SCANNER_CUDA_DEVICE_MEMORY_HPP
...
namespace vm 
{
	namespace scanner
	{
		...
		{
			/**
			 * \brief Error handler. All GPU functions from this subsystem call the function to report an error.
			...
			////////////////////////////////////////////////////////////////////////////////////////////////////////
			/**
			 * \brief DeviceMemory Class
			 *
			 * \note This is BLOB container class with reference counting for CPU memory
			 */
			class DeviceMemory
			{
			public:
				...
				/**
				 * \brief Initializes with user allocated buffer. Reference counting is disabled in this mode.
				...
				/**
				 * \brief Allocates internal buffer in GPU memory. 
				 * 				If internal buffer was created before the function recreates it with new size.
				 * 				if new and old size are same, it does nothing.
				 * \param sizeBytes_arg: buffer size; 
				 */
	    ...
	    private:
			...
			////////////////////////////////////////////////////////////////////////////////////////////////////////
			/**
			 * \brief DeviceMemory2D class
			 *
			 * \note This is BLOB container class with reference counting for pitched GPU memory
			 */
		...
		namespace device
		{
			using vm::scanner::cuda::DeviceMemory;
			using vm::scanner::cuda::DeviceMemory2D;
		}
	}
}
...
template<class T> inline 				T* vm::scanner::cuda::DeviceMemory::ptr() 			{ return ( 			 	T*)data_; }
template<class T> inline const 	T* vm::scanner::cuda::DeviceMemory::ptr() const { return ( const 	T*)data_; }

template<class U> inline vm::scanner::cuda::DeviceMemory::operator vm::scanner::cuda::PtrSz<U>() const
{
	PtrSz<U> result;
	result.data = (U*)ptr<U>();
...
template<class T>        T* vm::scanner::cuda::DeviceMemory2D::ptr(int y_arg)       { return (      T*)((      char*)data_ + y_arg * step_); }
template<class T>  const T* vm::scanner::cuda::DeviceMemory2D::ptr(int y_arg) const { return (const T*)((const char*)data_ + y_arg * step_); }

template <class U> vm::scanner::cuda::DeviceMemory2D::operator vm::scanner::cuda::PtrStep<U>() const
{
	PtrStep<U> result;
	result.data = (U*)ptr<U>();
	result.step = step_;
...
template <class U> vm::scanner::cuda::DeviceMemory2D::operator vm::scanner::cuda::PtrStepSz<U>() const
{
	PtrStepSz<U> result;
	result.data = (U*)ptr<U>();
	result.step = step_;
