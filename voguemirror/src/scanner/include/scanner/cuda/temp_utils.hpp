#ifndef VM_SCANNER_CUDA_TEMP_UTILS_HPP
#define VM_SCANNER_CUDA_TEMP_UTILS_HPP

#include <cuda.h>
#include <scanner/cuda/kernel_containers.hpp>
...
namespace vm
{
	namespace scanner
	{
      ...
      template <class T> __vm_hdevice__ void swap(T& a, T& b) { T c(a); a=b; b=c; }

      template<typename T> struct numeric_limits;

      template<> struct numeric_limits<float>
      {
        __vm_device__ static float quiet_NaN() { return __int_as_float(0x7fffffff); /*CUDART_NAN_F*/ };
        __vm_device__ static float epsilon() { return 1.192092896e-07f/*FLT_EPSILON*/; };
        __vm_device__ static float min() { return 1.175494351e-38f/*FLT_MIN*/; };
        __vm_device__ static float max() { return 3.402823466e+38f/*FLT_MAX*/; };
      };

      template<> struct numeric_limits<unsigned short>
      {
        __vm_device__ static unsigned short max() { return USHRT_MAX; };
      };

      __vm_device__ float dot(const float3& v1, const float3& v2)
      {
        return __fmaf_rn(v1.x, v2.x, __fmaf_rn(v1.y, v2.y, v1.z*v2.z));
      }

      __vm_device__ float3& operator+=(float3& vec, const float& v)
      {
        vec.x += v;  vec.y += v;  vec.z += v; return vec;
      }

      __vm_device__ float3& operator+=(float3& v1, const float3& v2)
      {
        v1.x += v2.x;  v1.y += v2.y;  v1.z += v2.z; return v1;
      }

      __vm_device__ float3 operator+(const float3& v1, const float3& v2)
      {
      ...
      __vm_hdevice__ float3 operator*(const float3& v1, const int3& v2)
      {
        return make_float3(v1.x * v2.x, v1.y * v2.y, v1.z * v2.z);
      }
      ...
      __vm_hdevice__ float3 operator/(const float& v, const float3& vec)
      {
      ...
      __vm_hdevice__ float3 operator*(const float3& v1, const float& v)
      {
        return make_float3(v1.x * v, v1.y * v, v1.z * v);
      }

      __vm_hdevice__ float3 operator*(const float& v, const float3& v1)
      {
        return make_float3(v1.x * v, v1.y * v, v1.z * v);
      }

      __vm_device__ float norm(const float3& v)
      {
        return sqrt(dot(v, v));
      }

      __vm_device__ float norm_sqr(const float3& v)
      {
        return dot(v, v);
      }

      __vm_device__ float3 normalized(const float3& v)
      {
        return v * rsqrt(dot(v, v));
      }

      __vm_hdevice__ float3 cross(const float3& v1, const float3& v2)
      {
        return make_float3(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x);
      }

      __vm_device__ void computeRoots2(const float& b, const float& c, float3& roots)
      {
        roots.x = 0.f;
        float d = b * b - 4.f * c;
      ...
      __vm_device__ void computeRoots3(float c0, float c1, float c2, float3& roots)
      {
      ...
      public:
        ...
        {
          float3 data[Rows];
          __vm_hdevice__ float3& operator[](int i) { return data[i]; }
          __vm_hdevice__ const float3& operator[](int i) const { return data[i]; }
        };
        typedef MiniMat<3> Mat33;
        typedef MiniMat<4> Mat43;


        static __vm_device__ float3 unitOrthogonal (const float3& src)
        {
          float3 perp;
        ...
        __vm_device__ Eigen33(volatile float* mat_pkg_arg) : mat_pkg(mat_pkg_arg) {}
        __vm_device__ void compute(Mat33& tmp, Mat33& vec_tmp, Mat33& evecs, float3& evals)
        {
      ...
      private:
        ...
        __vm_device__ float m00() const { return mat_pkg[0]; }
        __vm_device__ float m01() const { return mat_pkg[1]; }
        __vm_device__ float m02() const { return mat_pkg[2]; }
        __vm_device__ float m10() const { return mat_pkg[1]; }
        __vm_device__ float m11() const { return mat_pkg[3]; }
        __vm_device__ float m12() const { return mat_pkg[4]; }
        __vm_device__ float m20() const { return mat_pkg[2]; }
        __vm_device__ float m21() const { return mat_pkg[4]; }
        __vm_device__ float m22() const { return mat_pkg[5]; }

        __vm_device__ float3 row0() const { return make_float3( m00(), m01(), m02() ); }
        __vm_device__ float3 row1() const { return make_float3( m10(), m11(), m12() ); }
        __vm_device__ float3 row2() const { return make_float3( m20(), m21(), m22() ); }

        __vm_device__ static bool isMuchSmallerThan (float x, float y)
        {
      ...
      struct Warp
      {
        ...
        static __vm_device__ unsigned int laneId()
        {
          unsigned int ret;
        ...
        static __vm_device__ unsigned int id()
        {
          int tid = threadIdx.z * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
          return tid >> LOG_WARP_SIZE;
        }

        static __vm_device__ int laneMaskLt()
        {
        ...
        static __vm_device__ int binaryExclScan(int ballot_mask)
        {
      ...
      struct Block
      {
        static __vm_device__ unsigned int stride()
        {
          return blockDim.x * blockDim.y * blockDim.z;
        }

        static __vm_device__ int flattenedThreadId()
        {
          return threadIdx.z * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
        }

        template<int CTA_SIZE, typename T, class BinOp>
        static __vm_device__ void reduce(volatile T* buffer, BinOp op)
        {
        ...
        template<int CTA_SIZE, typename T, class BinOp>
        static __vm_device__ T reduce(volatile T* buffer, T init, BinOp op)
        {
          int tid = flattenedThreadId();
      ...
      struct Emulation
      {
        static __vm_device__ int warp_reduce ( volatile int *ptr , const unsigned int tid)
        {
        ...
        static __vm_device__ int Ballot(int predicate, volatile int* cta_buffer)
        {
#if __CUDA_ARCH__ >= 200
          (void)cta_buffer;
	      ...
	      static __vm_device__ bool All(int predicate, volatile int* cta_buffer)
        {
#if __CUDA_ARCH__ >= 200
          (void)cta_buffer;
