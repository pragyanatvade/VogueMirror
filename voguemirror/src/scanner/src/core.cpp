...
int vm::scanner::cuda::getCudaEnabledDeviceCount()
{
	int count;
...
void vm::scanner::cuda::setDevice(int device)
{
	cudaSafeCall( cudaSetDevice( device ) );
}

std::string vm::scanner::cuda::getDeviceName(int device)
{
	cudaDeviceProp prop;
...
bool vm::scanner::cuda::checkIfPreFermiGPU(int device)
{
  if (device < 0)
...
namespace 
{
  template <class T> inline void getCudaAttribute(T *attribute, CUdevice_attribute device_attribute, int device)
  {
    *attribute = T();
    ...
    vm::scanner::cuda::error("driver API error", __FILE__, __LINE__);
  }
...
void vm::scanner::cuda::printCudaDeviceInfo(int device)
{
...
void vm::scanner::cuda::printShortCudaDeviceInfo(int device)
{
    int count = getCudaEnabledDeviceCount();
    bool valid = (device >= 0) && (device < count);
...
vm::scanner::SampledScopeTime::SampledScopeTime(double& time_ms) : time_ms_(time_ms)
{
    start = (double)cv::getTickCount();
}
vm::scanner::SampledScopeTime::~SampledScopeTime()
{
    static int i_ = 0;
    time_ms_ += getTime ();
    if (i_ % EACH == 0 && i_)
...
double vm::scanner::SampledScopeTime::getTime()
{
    return ((double)cv::getTickCount() - start)*1000.0/cv::getTickFrequency();
}

vm::scanner::ScopeTime::ScopeTime(const char *name_) : name(name_)
{
    start = (double)cv::getTickCount();
}
vm::scanner::ScopeTime::~ScopeTime()
{
