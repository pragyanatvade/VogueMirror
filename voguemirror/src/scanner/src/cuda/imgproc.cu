...
void vm::scanner::device::bilateralFilter (const Depth& src, Depth& dst, int kernel_size, float sigma_spatial, float sigma_depth)
{
...
void vm::scanner::device::truncateDepth(Depth& depth, float max_dist /*meters*/)
{
  dim3 block (32, 8);
...
void vm::scanner::device::depthPyr(const Depth& source, Depth& pyramid, float sigma_depth)
{
  sigma_depth *= 1000; // meters -> mm
...
void vm::scanner::device::computeNormalsAndMaskDepth(const Reprojector& reproj, Depth& depth, Normals& normals)
{
  dim3 block (32, 8);
  dim3 grid (divUp (depth.cols (), block.x), divUp (depth.rows (), block.y));
...
void vm::scanner::device::computePointNormals(const Reprojector& reproj, const Depth& depth, Points& points, Normals& normals)
{
    dim3 block (32, 8);
    dim3 grid (divUp (depth.cols (), block.x), divUp (depth.rows (), block.y));
...
void vm::scanner::device::compute_dists(const Depth& depth, Dists dists, float2 f, float2 c)
{
  dim3 block (32, 8);
  dim3 grid (divUp (depth.cols (), block.x), divUp (depth.rows (), block.y));
...
void vm::scanner::device::resizeDepthNormals(const Depth& depth, const Normals& normals, Depth& depth_out, Normals& normals_out)
{
...
void vm::scanner::device::resizePointsNormals(const Points& points, const Normals& normals, Points& points_out, Normals& normals_out)
{
...
void vm::scanner::device::renderImage(const Depth& depth, const Normals& normals, const Reprojector& reproj, const float3& light_pose, Image& image)
{
    dim3 block (32, 8);
    dim3 grid (divUp (depth.cols(), block.x), divUp (depth.rows(), block.y));
...
void vm::scanner::device::renderImage(const Points& points, const Normals& normals, const Reprojector& reproj, const Vec3f& light_pose, Image& image)
{
    dim3 block (32, 8);
...
void vm::scanner::device::renderTangentColors(const Normals& normals, Image& image)
{
  dim3 block (32, 8);
...
namespace vm
{
  namespace scanner
  {
    namespace device
    {
      ...
      __global__ void vertex_colors_kernel(const PtrStep<Point> points, const PtrStepSz<Normal> normals,
                                            const Reprojector reproj, const float3 light_pose, 
                                            const PtrStepSz<uchar4> textures, PtrStepSz<uchar4> colors)
      {
        int x = threadIdx.x + blockIdx.x * blockDim.x;
        int y = threadIdx.y + blockIdx.y * blockDim.y;
        ...
        if(isnan(p.x))
        {
          ...
          float w = static_cast<float>(y) / colors.rows;
          color = bgr1 * (1 - w) + bgr2 * w;
        ...
        else
        {
          float3 P = p;
          float3 N = tr(normals(y, x));
          uchar4 t = textures(y, x);
          float3 T = make_float3(t.x/255.f, t.y/255.f, t.z/255.f);

          const float Ka = 0.1f;  //ambient coeff
          const float Kd = 0.7f;  //diffuse coeff
          const float Ks = 0.2f;  //specular coeff
          const float n = 20.f;   //specular power

          const float3 A = make_float3(1.f, 1.f, 1.f);   // ambient color
          const float3 D = make_float3(T.x, T.y, T.z);  // diffuse color
          const float3 S = make_float3(1.f, 1.f, 1.f);   // specular color
          const float3 LC = make_float3(1.5, 1.5, 1.5);   // light color

          float3 L = normalized(light_pose - P);
          float3 V = normalized(make_float3(0.f, 0.f, 0.f) - P);
          float3 R = normalized(2 * N * dot(N, L) - L);

          float Ix = A.x*Ka*D.x + LC.x * Kd * D.x * fmax(0.f, dot(N, L)) + LC.x * Ks * S.x * __powf(fmax(0.f, dot(R, V)), n);
          float Iy = A.y*Ka*D.y + LC.y * Kd * D.y * fmax(0.f, dot(N, L)) + LC.y * Ks * S.y * __powf(fmax(0.f, dot(R, V)), n);
          float Iz = A.z*Ka*D.z + LC.z * Kd * D.z * fmax(0.f, dot(N, L)) + LC.z * Ks * S.z * __powf(fmax(0.f, dot(R, V)), n);
...
void vm::scanner::device::renderVertexColors(const Points& points, const Normals& normals, const Reprojector& reproj, const Vec3f& light_pose, const Image& colors, Image& image)
{
  dim3 block (32, 8);
  dim3 grid (divUp (normals.cols(), block.x), divUp (normals.rows(), block.y));

  vertex_colors_kernel<<<grid, block>>>((PtrStep<Point>)points, normals, reproj, light_pose, colors, image);
  cudaSafeCall ( cudaGetLastError () );
}
...
void vm::scanner::device::mergePointNormal (const DeviceArray<Point>& cloud, const DeviceArray<float8>& normals, const DeviceArray<float12>& output)
{
  const int block = 256;
