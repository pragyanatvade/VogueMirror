...
void vm::scanner::device::ComputeIcpHelper::operator()(const Depth& dprev, const Normals& nprev, DeviceArray2D<float>& buffer, float* data, cudaStream_t s)
{
...
void vm::scanner::device::ComputeIcpHelper::operator()(const Points& vprev, const Normals& nprev, DeviceArray2D<float>& buffer, float* data, cudaStream_t s)
{
  dprev_tex.filterMode = cudaFilterModePoint;
  nprev_tex.filterMode = cudaFilterModePoint;
...
void vm::scanner::device::ComputeIcpHelper::allocate_buffer(DeviceArray2D<float>& buffer, int partials_count)
{ 
  if (partials_count < 0)
  {
...
vm::scanner::device::ComputeIcpHelper::PageLockHelper::PageLockHelper() : data(0)
{ cudaSafeCall( cudaMallocHost((void **)&data, Policy::TOTAL * sizeof(float)) );  }

vm::scanner::device::ComputeIcpHelper::PageLockHelper::~PageLockHelper()
