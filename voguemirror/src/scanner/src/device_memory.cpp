...
void vm::scanner::cuda::error(const char* error_string, const char* file, const int line, const char* func)
{
	std::cout << "Scanner Error: " << error_string << "\t" << file << ":" << line << std::endl;
	exit(0);
}
...
vm::scanner::cuda::DeviceMemory::DeviceMemory() : data_(0), sizeBytes_(0), refcount_(0) {}
vm::scanner::cuda::DeviceMemory::DeviceMemory(void* ptr_arg, size_t sizeBytes_arg) : data_(ptr_arg), sizeBytes_(sizeBytes_arg), refcount_(0) {}
vm::scanner::cuda::DeviceMemory::DeviceMemory(size_t sizeBytes_arg) : data_(0), sizeBytes_(0), refcount_(0) { create(sizeBytes_arg); };
vm::scanner::cuda::DeviceMemory::~DeviceMemory() { release(); }
...
void vm::scanner::cuda::DeviceMemory::copyTo(DeviceMemory& other) const
{
    if (empty())
        other.release();
    else
...
void vm::scanner::cuda::DeviceMemory::swap(DeviceMemory& other_arg)
{
...
bool vm::scanner::cuda::DeviceMemory::empty() const { return !data_; }
size_t vm::scanner::cuda::DeviceMemory::sizeBytes() const { return sizeBytes_; }

////////////////////
// DeviceMemory2D //
////////////////////

vm::scanner::cuda::DeviceMemory2D::DeviceMemory2D() : data_(0), step_(0), colsBytes_(0), rows_(0), refcount_(0) {}

vm::scanner::cuda::DeviceMemory2D::DeviceMemory2D(int rows_arg, int colsBytes_arg)
    : data_(0), step_(0), colsBytes_(0), rows_(0), refcount_(0)
{ 
    create(rows_arg, colsBytes_arg); 
}

vm::scanner::cuda::DeviceMemory2D::DeviceMemory2D(int rows_arg, int colsBytes_arg, void *data_arg, size_t step_arg)
    :  data_(data_arg), step_(step_arg), colsBytes_(colsBytes_arg), rows_(rows_arg), refcount_(0) {}

vm::scanner::cuda::DeviceMemory2D::~DeviceMemory2D() { release(); }
...
void vm::scanner::cuda::DeviceMemory2D::release()
{
    if( refcount_ && CV_XADD(refcount_, -1) == 1 )
    {
        //cv::fastFree(refcount);
...
void vm::scanner::cuda::DeviceMemory2D::swap(DeviceMemory2D& other_arg)
{    
    std::swap(data_, other_arg.data_);
...
bool vm::scanner::cuda::DeviceMemory2D::empty() const { return !data_; }
int vm::scanner::cuda::DeviceMemory2D::colsBytes() const { return colsBytes_; }
int vm::scanner::cuda::DeviceMemory2D::rows() const { return rows_; }
size_t vm::scanner::cuda::DeviceMemory2D::step() const { return step_; }
