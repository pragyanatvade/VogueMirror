...
vm::scanner::device::ComputeIcpHelper::ComputeIcpHelper(float dist_thres, float angle_thres)
{
...
/////////////////////////////////
struct vm::scanner::cuda::ProjectiveICP::StreamHelper
{
...
// ProjectiveICP //
///////////////////
vm::scanner::cuda::ProjectiveICP::ProjectiveICP() : angle_thres_(deg2rad(20.f)), dist_thres_(0.1f)
{ 
...
vm::scanner::cuda::ProjectiveICP::~ProjectiveICP() {}

float vm::scanner::cuda::ProjectiveICP::getDistThreshold() const
{ return dist_thres_; }

void vm::scanner::cuda::ProjectiveICP::setDistThreshold(float distance)
{ dist_thres_ = distance; }

float vm::scanner::cuda::ProjectiveICP::getAngleThreshold() const
{ return angle_thres_; }

void vm::scanner::cuda::ProjectiveICP::setAngleThreshold(float angle)
{ angle_thres_ = angle; }

void vm::scanner::cuda::ProjectiveICP::setIterationsNum(const std::vector<int>& iters)
{
  if (iters.size() >= MAX_PYRAMID_LEVELS)
    iters_.assign(iters.begin(), iters.begin() + MAX_PYRAMID_LEVELS);
  else
  {
    iters_ = std::vector<int>(MAX_PYRAMID_LEVELS, 0);
    copy(iters.begin(), iters.end(),iters_.begin());
  }
}

int vm::scanner::cuda::ProjectiveICP::getUsedLevelsNum() const
{
...
bool vm::scanner::cuda::ProjectiveICP::estimateTransform(Affine3f& /*affine*/, const Intr& /*intr*/, const Frame& /*curr*/, const Frame& /*prev*/)
{
...
bool vm::scanner::cuda::ProjectiveICP::estimateTransform(Affine3f& affine, const Intr& intr, const DepthPyr& dcurr, const NormalsPyr ncurr, const DepthPyr dprev, const NormalsPyr nprev)
{
